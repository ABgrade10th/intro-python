...
# clues
#we're using a while loop- program will repeat itsself; if 
# the b=number is not the correct guess.
# we'll need to create a function to store it.
# we'll use the input function to take the users guess.

def guessingGame():
    randomNumber = randon.randint(1, 10)
# print(f'Random number valueis:{randomNumber}')
    i = int(input('please guess a number:'))

    while i == randomNumber: # while this true...
